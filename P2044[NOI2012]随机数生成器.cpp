...
Matrix<M,P,T> Matrix<M,N,T>::operator*(Matrix<N,P,T> in){
	...
	for(int i=0;i<M;i++)
		for(int j=0;j<N;j++)
			for(int k=0;k<P;k++)
				ADD.Data[i][k]=((long long)ADD.Data[i][k]+mul(Data[i][j],in.Data[j][k]))%mod;
	return ADD;
}
...
Matrix<2,2,long long> a0,ans,k[70];
Matrix<2,1,long long> t;
int main(){
	scanf("%lld%lld%lld%lld%lld%lld",&mod,&a,&c,&x,&n,&g);
	
	a0.Data[0][0]=a;a0.Data[0][1]=c;
	a0.Data[1][0]=0;a0.Data[1][1]=1;
	t.Data[0][0]=x;t.Data[1][0]=1;
