...
int tt[M],lb[M];
void polyexp(int len,int *a,int *b){
    if(len==1){
        b[0]=1;
        return;
    }
    ...
    int l=1;
    while(l<=(len<<1))l<<=1;
    for(int i=0;i<len;i++)tt[i]=a[i];
    for(int i=len;i<l;i++)tt[i]=0;
    lim=l;
    
    for(int i=0;i<l;i++)lb[i]=b[i];
    for(int i=l;i<2*l;i++)lb[i]=0;
    polyln(l,lb);
    for(int i=0;i<l;i++)lb[i]=((1+tt[i]-lb[i])%p+p)%p;
    NTT(b,1),NTT(lb,1);
    for(int i=0;i<l;i++)
        b[i]=1ll*lb[i]*b[i]%p;
    NTT(b,-1);
