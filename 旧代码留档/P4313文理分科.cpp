...
int main(){
    ...
    for(int i=1;i<=n;i++)   
        for(int j=1;j<=m;j++){
            ...
            if(i<n)add(num(i,j,1),num(i+1,j,0),inf);
            if(j<m)add(num(i,j,1),num(i,j+1,0),inf);
            if(i>1)add(num(i,j,1),num(i-1,j,0),inf);
            if(j>1)add(num(i,j,1),num(i,j-1,0),inf);
        }
    for(int i=1;i<=n;i++)   
        for(int j=1;j<=m;j++){
            int x;scanf("%d",&x);ans+=x;
            ...
            if(i<n)add(num(i+1,j,0),num(i,j,2),inf);
            if(j<m)add(num(i,j+1,0),num(i,j,2),inf);
            if(i>1)add(num(i-1,j,0),num(i,j,2),inf);
            if(j>1)add(num(i,j-1,0),num(i,j,2),inf);
        }
    printf("%d\n",ans-dinic());
}