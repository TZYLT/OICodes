#include<bits/stdc++.h>
using namespace std;

#define M (1<<20)
#define p 998244353
#define g 3
long long qpow(long long a,long long k){
    long long res=1;
    while(k){
        if(k&1)
            res=(res*a)%p;
        a=(a*a)%p;
        k>>=1;
    }
    return res%p;
}
int rev[M],len,lim;
void NTT(int* a,int opt){
    len=__lg(lim);
    for(int i=0;i<lim;i++)
        rev[i]=(rev[i>>1]>>1)|((i&1)<<(len-1));
    //Warning:rev[] may been calculated many times
    for(int i=0;i<lim;++i)
        if(i<rev[i])
            swap(a[i],a[rev[i]]);
    for(int i=1;i<=len;i++){
        int m=1<<i;
        int wn=qpow(opt==1?g:qpow(g,p-2),(p-1)/m);
        for(int k=0;k<lim;k+=m){
            int w=1;
            for(int j=0;j<m/2;++j){
                int t=(long long)w*a[k+j+m/2]%p;
                int u=a[k+j];
                a[k+j]=((long long)u+t)%p;
                a[k+j+m/2]=((long long)u-t+p)%p;
                w=(long long)w*wn%p;
            }
        }
    }
    if(opt==-1){
        int inv=qpow(lim,p-2);
        for(int i=0;i<lim;i++)
            a[i]=(long long)a[i]*inv%p;
    }        
}
int t[M];
void polyinv(int len,int *a,int *b){
    if(len==1){
        b[0]=qpow(a[0],p-2);
        return;
    }
    polyinv((len+1)>>1,a,b);
    int l=1;
    while(l<=(len<<1))l<<=1;
    for(int i=0;i<len;i++)t[i]=a[i];
    for(int i=len;i<l;i++)t[i]=0;
    lim=l;
    NTT(t,1),NTT(b,1);
    for(int i=0;i<l;i++)
        b[i]=1ll*(2-1ll*t[i]*b[i]%p+p)%p*b[i]%p;
    NTT(b,-1);
    for(int i=len;i<l;i++)b[i]=0;
}
int inv[M],fac[M],ifac[M];
void inv_liner(int n){
    inv[1]=fac[0]=ifac[0]=1;
    for(int i=2;i<=n;++i)
        inv[i]=(long long)(p-p/i)*inv[p%i]%p;
    for(int i=1;i<=n;++i)
        fac[i]=1ll*fac[i-1]*i%p,
        ifac[i]=1ll*ifac[i-1]*inv[i]%p;
}
void polyint(int len,int *a){
    for(int i=len;i>=1;i--)
        a[i]=(1ll*a[i-1]*inv[i])%p;
    a[0]=0;
}
void polyderive(int len,int *a){
    for(int i=0;i<len;i++)
        a[i]=(1ll*a[i+1]*(i+1))%p;
}
int pinv[M];
void polyln(int len,int *a){
	memset(pinv,0,sizeof(pinv));
    polyinv(len,a,pinv);
    polyderive(len,a);
    lim=1;
    while(lim<=(len<<1))lim<<=1;
    lim<<=1;
    NTT(a,1);
    NTT(pinv,1);
    for(int i=0;i<lim;i++)
        a[i]=(1ll*a[i]*pinv[i])%p;
    NTT(a,-1);
    polyint(len,a);
}
void polymul(int len,int *a,int *b){
    lim=(1<<(__lg(len)+1));
    NTT(a,1);
    NTT(b,1);
    
    for(int i=0;i<lim;i++)
        a[i]=1ll*a[i]*b[i]%p;
    NTT(a,-1);
}
void mg(int l,int r,int *x,int *w){
    if(l==r){
		x[0]=1;x[1]=p-w[l];
		return;
	}
    int mid=(l+r)>>1;
    int y[M];
    for(int i=0;i<=(1<<(__lg(r-l+1)+2));i++)
        y[i]=0;
    mg(l,mid,x,w);
    mg(mid+1,r,y,w);
    lim=(1<<(__lg(r-l+1)+2));
    NTT(x,1);
    NTT(y,1);
    for(int i=0;i<lim;i++)
        x[i]=1ll*x[i]*y[i]%p;
    NTT(x,-1);
}
int n,m,tt,k,a[M],b[M],sa[M],sb[M],s[M];
int main(){
	inv_liner(M-10);
    scanf("%d%d",&n,&m);
    for(int i=0;i<n;i++)
        scanf("%d",a+i);
    for(int i=0;i<m;i++)
        scanf("%d",b+i);
    scanf("%d",&tt);
    k=tt,k=max(max(k,n),m);
    k++;
	mg(0,n-1,sa,a);
	mg(0,m-1,sb,b);
    polyln(k,sa);
    polyln(k,sb);
    polyderive(k,sa);
    polyderive(k,sb);
    for(int i=k;i>0;i--){
        sa[i]=p-sa[i-1];
        sb[i]=p-sb[i-1];
    }
    sa[0]=n;
    sb[0]=m;
    for(int i=0;i<=k;i++){
        sa[i]=1ll*sa[i]*ifac[i]%p;
        sb[i]=1ll*sb[i]*ifac[i]%p;
    }
    polymul(k<<1,sa,sb);
    int tmp=qpow(1ll*n*m%p,p-2);
    for(int i=0;i<=k;i++)
        sa[i]=1ll*sa[i]*fac[i]%p;
    for(int i=1;i<=tt;i++)
        printf("%lld\n",1ll*sa[i]*tmp%p);
}